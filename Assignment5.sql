...
from employee INNER JOIN emplevel ON employee.salary>=emplevel.lowsalary and employee.salary<=emplevel.highsalary;
...
-- Ques 5.
select concat(employee.lname, concat(',',employee.fname)) as "Employee Name", dept.deptname, position.poscdesc,qualification.qualdesc from Employee INNER JOIN Dept
    ON Employee.deptid=dept.deptid INNER JOIN position ON Employee.PositionID=Position.PositionID 
...
-- Ques 7.
select concat(employee.lname, concat(',',employee.fname)) as "Employee Name",dependent.relation 
...
dept ON employee.deptid = dept.deptid where dept.deptid=(select employee.deptid from employee where employee.fname='John') AND fname != 'John';
...
-- Ques 12
select concat(student.fname, concat(',',Student.lname)) as "Student Name", FACULTY.NAME, LOCATION.BUILDING,LOCATION.ROOMNO 
from STUDENT INNER JOIN FACULTY ON FACULTY.FACULTYID = STUDENT.FACULTYID INNER JOIN 
...
FACULTY on CRSSECTION.FACULTYID=FACULTY.FACULTYID FULL JOIN TERM on CRSSECTION.TERMID=TERM.TERMID where startdate between
